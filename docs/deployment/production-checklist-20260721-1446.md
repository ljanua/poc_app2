...
### This run focus (2026-07-21 — deploy latest DB + code on `main`)

**Release commit:** `0f85ee9` (`feat: S6 focus expand, S7 admin controls, and scout SWOT debugging`).

**Migration inventory:** highest file remains `034_registration_intent_and_join_requests.sql`. **No new `NNN_*.sql` since the 2026-07-20 checklist.** Schema work for this run is closing any remaining **033 / 034** gaps (and earlier 025–032 if still missing).

If 025–032 are already on prod, apply only:
...
**Code that depends on this schema (after restart):** S7 Approvals & Tiers (approved-only Users list, change subscription), public registration intent / OAuth handoff. S6 focus-expand + video SWOT/DEBUG are **code-only** (no new columns).
...
- [x] Gap list written down (only missing migrations will be applied).
  - **2026-07-21 verify on `vantagiq`:** 025, 029, 031, 032, 033 (tables + user cols + 4 tiers), 034 all **present**. Gap list: **empty**.
...
| S7 Users (approved-only) | Users tab lists approved/active users only; pending not mixed into Users |
| S7 Change subscription | Admin can change a user’s `subscription_tier_id` without 500 |
| Registration intent | Public create/join wizard persists intent; SA approve materializes club/team |
| S6 assessments | Expand details → focus mode + full skill names; Collapse restores grid |
...
- [x] Note date, commit SHA, migrations applied, who applied them.
  - **Date:** 2026-07-21
  - **Commit:** `0f85ee9`
  - **DB:** `vantagiq` (via `.env_prod`)
  - **Migrations applied this run:** none (verify showed 025–034 already present; 033/034 closed)
  - **Mode:** full — schema verified; code restart is operator follow-up if prod host is separate
- [x] Note any skipped opt-in scripts. — purge scripts skipped
- [ ] If something failed mid-apply, leave a short note for the next deploy (which file failed, error text). — N/A
