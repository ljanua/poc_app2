...
| S7-admin-user-management.html | Filter by role/status/search | List users | GET /v1/users | 200 OK with approved-only list (`approval_status = active`; includes `subscriptionTierId`, `tierCode`, `tierDisplayName`; last login remains on payload for Status tooltip) | 403 forbidden |
| S7-admin-user-management.html | Change Subscription modal submit (SystemAdmin only) | Change user subscription | POST /v1/admin/users/{userId}/subscription | 200 OK with updated user (`subscriptionTierId`, `tierCode`, `tierDisplayName`, role synced via `roleForTierCode` except SystemAdmin targets keep SystemAdmin) | 400 validation_error, 403 forbidden, 404 not_found |
...
| S2-player-dashboard.html | Share link (toolbar icon 🔗) | Create / revoke guest share | POST /v1/players/{playerId}/share ; DELETE /v1/players/{playerId}/share ; GET status | 200 with one-time `{ token, url }` (hash stored); replace-on-create; Coach lead-coach or SystemAdmin | 403 forbidden, 404 not_found |
...
| S10-games.html | List / create fixtures | Games CRUD | GET/POST /v1/games ; GET/PATCH /v1/games/{gameId} | 200/201 with `{ games }` / `{ game }`; Coach/ClubAdmin club-scoped; SystemAdmin unrestricted. Create Duration presets from the selected team’s sport `durationMinutes`. | 400 validation_error, 403 forbidden, 404 not_found |
| S10-games.html | Save Game Sheet | Complete sheet + rollup | PUT /v1/games/{gameId}/sheet | 200 with `{ game, substitutions, performances, minutesByPlayer }`; event-sourced minutes; upserts `game_performance`; recomputes `player_stats` match fields; starter count capped by team sport `numberOfPlayers`; does **not** write skill ratings / Assessment history | 400 validation_error, 403 forbidden, 404 not_found |
| S2-player-dashboard.html | Match History Performance | List completed games for player | GET /v1/players/{playerId}/match-history?actorEmail= | 200 with `{ events: [{ gameId, kickoffAt, opponent, homeAway, minutes, rating, started }] }` newest-first; section `data-testid="match-history-performance-section"` replaces Match Time History | 403 forbidden |
...
| S8-skills.html | Add Sport modal submit | Create sport | POST /v1/sports | 201 Created with sanitized sport object (`status: 'active'`, `durationMinutes`, `numberOfPlayers`) | 400 validation_error, 403 forbidden, 409 conflict |
| S8-skills.html | Rename Sport modal submit | Update sport | PATCH /v1/sports/{sportId} | 200 OK with updated sport including `durationMinutes` and `numberOfPlayers` | 400 validation_error, 403 forbidden, 404 not_found, 409 conflict |
...
- **S4 Link ingest (backlog 018) / analysis window** — source mode Upload | From Link. **Both** modes post multipart fields `startMmSs`, `durationMmSs` (default `00:30`, max `00:30` / 30 seconds) plus `findPlayer`; link mode also posts `videoUrl` instead of a video file. Backend downloads via yt-dlp (hosted) or HTTP (direct), then extracts Start+Duration re-encoded at **≤ 30 fps**. When `findPlayer=true` (link path), Ollama match runs **only inside that extract**; on match, assessment uses a further trim from the match offset (absolute `find_player_matched_ms` = start + offset). File uploads trim the same Start+Duration window before segmentation/AI. Durable AI segments are **10 seconds** each, re-encoded at **480p**, **audio stripped (`-an`)**, **max 3 segments** (≤30s analyzed). Columns: `source_url`, `source_start_ms`, `source_duration_ms`, `find_player`, `find_player_matched_ms`. Link mode requires backend (`DATABASE_URL`); offline mock keeps file upload only. UI: `[data-testid="source-mode-link"]`, `[data-testid="analysis-window-alert"]`, `[data-testid="find-player-toggle"]` (default ON when avatar present).

Dashboard "no stats yet" contract (player found, but `player_stats` has no real data):
- `missingDataMessage` (surfaced at `data.performance.missingDataMessage`, mirrored onto `PlayerDashboardStats.missingDataMessage`) is the single authoritative signal that this player has no genuinely recorded growth/performance stats yet. On S2 it hides **Development Progress**, **Match History Performance**, and **Recent Performance**, and shows a single notice next to the player identity card (name, team, position, trend badge remain visible).
...
- Clips carry `comments` (LLM video observation on successful assessment; on failure the same text as `error_message`). Video assessment prompts use an elite soccer scout profile; the model must return JSON `ratings` plus **required** non-empty `strengths` / `weaknesses` / `opportunities` arrays (optional one-sentence `comments` coda). The worker formats SWOT into sectioned `comments` (STRENGTHS / WEAKNESSES / OPPORTUNITIES); if SWOT arrays are missing it retries once and logs `ollama.swot_incomplete` / `ollama.swot_incomplete_after_retry`. Clip `summary` is a derived short line (comments note + skill ratings), not the primary scout text. `GET /api/v1/clips` and upload responses include `comments`. S6 assessment cards show overall score then a **truncated comment preview** when collapsed; preview + **Expand / Collapse details** share `.assessment-expand-toolbar` with the toggle **right-aligned**. **Expand details** enters **focus mode** (`#resultsGrid.assessment-focus-mode`): hides other cards, widens the selected card horizontally, and shows full `comments` + skills (`[data-testid="assessment-details"]`). **Collapse details** restores the multi-card grid.
...
- **S6 Open original + Re-process** — `GET /v1/clips` and share clip payloads include `sourceUrl` when the clip was link-ingested. S6 cards with a non-empty `sourceUrl` show **Open original** (`[data-testid="open-original-link"]`) opening that URL in a new tab (`target="_blank"` + `rel="noopener noreferrer"`). **Re-process** (`[data-testid="reprocess-clip"]`) shows for signed-in Coach / ClubAdmin / SystemAdmin on **failed or complete/assessed** clips that have a usable `path` **or** `sourceUrl` (hidden when neither; guests never see it; pending excluded). Complete cards keep **Back** and add Re-process beside it when eligible. `POST /v1/clips/{id}/reprocess` with `{ actorEmail }` resets eligible clips to `submitted` (preserves `video_storage_path` / `source_*`; when path is empty sets `find_player=false`) and triggers the queue. Processing: existing on-disk path → **assess-only** (no download/extract/Find player); missing path + `sourceUrl` → extract Start+Duration (≤30 fps) **without** Find player, then assess.
- **S6 Delete** — editors (Coach / ClubAdmin / SystemAdmin) see **Delete** (`[data-testid="delete-clip"]`) on every card (any status). Guests never. Confirm via `window.confirm`, then `DELETE /api/v1/clips/{clipId}` with `{ actorEmail }` (204). Hard delete removes the clip row (`clip_segments` cascade), reconciles player clip stats, and best-effort unlinks media/thumbnail/segments paths under the video root.
- **S6 card skills + Back** — complete cards list assessment skills from `skillFocus` ∪ `skillRatings` keys (falling back to `skill` when both are empty), each as **full skill name** + percent or **N/A**, inside the expandable details region (hidden until Expand). Collapsed cards keep overall % + comment preview only. Expand is **exclusive focus** (one wide card at a time; siblings hidden). Guest share can expand the same way. The card action that returns to S2 is labeled **Back** (S2’s deep-link into S6 remains **View Results**).
- **S2 → S6 View Results** — the Video Assessments **View Results** control deep-links to `S6-assessment-list.html?playerId=&playerName=&teamName=` for the dashboard player. Guest S2 also appends `share=<token>`; guest S6 loads via share-scoped clip/media APIs and locks Pre-Selected/team. S6 shows a toggleable **Pre-Selected Player** checkbox (`[data-testid="preselected-player-filter"]`) that defaults **ON** when those params are present (OFF / hidden when opening S6 without player context; always on+locked for guests). Team defaults to the player's `teamName` when it appears in the allowed team list. Coach and ClubAdmin load teams via `GET /v1/teams?actorEmail=` (club-scoped); **All Teams** means all teams in the actor's club(s). SystemAdmin keeps a broader team list. `GET /v1/clips` accepts optional `actorEmail`, `playerId` / `playerName`, `teamName`, and `status`. When `actorEmail` resolves to an active Coach or ClubAdmin, clips are limited to players on teams in that user's `coach_clubs`; SystemAdmin (or omitted actor) stays unscoped; unknown/non-admin `actorEmail` fails closed to an empty list. Offline `MockupApi.listClips` mirrors the same rule (defaults `actorEmail` from session when omitted).
...
- S8 Sport filter (`#positionSportFilter`) defaults via `MockupApi.resolveDefaultSportId`: SystemAdmin → Soccer; Coach/ClubAdmin → primary club (`coach_clubs.created_at` ASC) `defaultSportId`, with Soccer/first-active fallback. Skills tab has a separate Sport filter (`#skillSportFilter` / `[data-testid="skill-sport-filter"]`) with the same default plus an **All sports** option; when a sport is selected, `GET /v1/skills?sportId=` returns only skills assigned under that sport (via `position_skills` → `positions`) and scopes `# Positions` counts accordingly. Skills/Sports nav is visible to SystemAdmin, ClubAdmin, and Coach; catalog writes remain SystemAdmin-only (read-only notice for club users).
...
- S7 Users table shows **Email under Name** and **Subscription under Role** in stacked cells (`tierDisplayName` / `tierCode` via `[data-testid="subscription-cell"]`) for SystemAdmin and ClubAdmin viewers — no separate Email/Subscription column headers. **Change Subscription** is SystemAdmin-only (`POST /v1/admin/users/{userId}/subscription`); on success the user’s role is set via `roleForTierCode` unless the target is already SystemAdmin (tier updates, role preserved).
- S7 Users tab lists **approved users only** (`GET /v1/users` filters `approval_status = 'active'`; pending stay on Approvals). The Approval and Last Login columns are removed; Status badge `[data-testid="user-status"]` carries `title="Last login: …"`.
...
- Playwright must not create extra positions under `sport_soccer`; use a disposable QA sport instead. Purge orphans with `node scripts/purge-soccer-position-orphans.js` (allowlist in `tests/playwright/_soccer-positions.js`). Purge automated-test leftover skills (`name` ILIKE `QA%` or abbreviation `QA%`, excluding the 31 migration-015 seed ids) with `node scripts/purge-qa-skills.js` (allowlist in `tests/playwright/_soccer-skills.js`). `tests/playwright/s8-skills.spec.js` `afterAll` runs both purges; Assign Skills tears down its `pos_gk` QA assignments before exit.
...
- `MockupApi.getCurrentUser()` and `MockupApi.applyRoleGatedNav(currentUser)` are the new entry points used by every mockup page:
  ...
  - The Clubs nav item carries `data-role-visible-to="SystemAdmin"`; the Users nav item carries `data-role-visible-to="SystemAdmin,ClubAdmin"` so coaches never see admin-only chrome.
...
- Every bottom-nav page (`S1`, `S2`, `S3`, `S3a`, `S4`, `S5`, `S6`, `S7`, `S7a`, `S8`) carries a `Users` nav entry gated to `SystemAdmin,ClubAdmin` (`data-testid="nav-users"`) linking to `S7-admin-user-management.html`, and a `Clubs` nav entry gated to `SystemAdmin` linking to `S7a-clubs.html`. Both ship `hidden` so coaches never see them before `MockupApi.applyRoleGatedNav` runs.
...
- **S3a** (`docs/ux/mockup/S3a-team-update.html`) is a form-only update screen: name, age group, lead coach, club, status, and sport (`#updateSportSelect`, `[data-testid="update-sport-select"]`) sourced from `MockupApi.listSports(...)` with the team's current sport preselected. There is no separate Current Snapshot panel. The submit handler forwards `sportId` (and the other fields) to `MockupApi.updateTeamCoachAndClub`; omitting `sportId` preserves the team's current sport.
...
- **S2 Skill Ratings groups** — the `Skill Ratings` heading is a static section title (not collapsible as a whole). Each position group collapses independently: `[data-testid="skill-ratings-any-toggle"]` / `[data-testid="skill-ratings-role-toggle"]` toggle `is-collapsed` on their `.skill-position-group` (body hidden via `.position-body`). Groups default **collapsed** on first visit; each group's expand/collapse state persists per player in the same `vantageiq_s2_dashboard_sections` object using slugs `skill-any` / `skill-role` (`true` = expanded, absent/`false` = collapsed), without clobbering the section slugs. Each group title shows a right-aligned bright-yellow (`#facc15`) average — `[data-testid="skill-ratings-any-average"]` / `[data-testid="skill-ratings-role-average"]` — computed as the mean of numeric ratings `> 0`, rounded (same formula as the S1 player card overall rating); `—` when no qualifying rating. The average renders on the always-visible title row regardless of collapsed state.
- **S2 collapsible sections** — each remaining stats block title (`Development Progress`, `Match History Performance`, `Recent Performance`, `Video Assessments`) is a `[data-testid="dashboard-section-toggle-*"]` button that toggles `is-collapsed` on the parent `.stats-section` (body hidden via `.section-body`). Default **collapsed** on first visit; expanded/collapsed state persists per player in `localStorage` key `vantageiq_s2_dashboard_sections` (`{ [playerId]: { [sectionSlug]: true|false } }`, `true` = expanded). Slug `match-history` replaces former `match-time`. Query `?section=video-assessments` forces that section open on load (and persists expanded).
- **S2 → S4 → S2 return** — dashboard clip CTAs link to `S4-video-capture.html?playerId=&from=s2`. When `from=s2` is present, successful submit and Cancel navigate to `S2-player-dashboard.html?player=<name>&section=video-assessments` (player name from the S4 selection). S4 without `from=s2` keeps in-place reset / `history.back()`.
- **S10 Games** — bottom-nav `data-testid="nav-games"`; list + create fixture + Game Sheet (`starters`, ordered substitutions out→in@minute, ratings 0–10). Minutes from `computeMinutesFromSheet` / `MockupApi.computeGameMinutes`. Offline store keys: `games`, `gameSubstitutions`, `gamePerformances`.
- **S2 toolbar** — Edit / Share / Revoke render as icon buttons (`.btn-icon`) with their labels carried by `title` + `aria-label` on a single row; the team dropdown and "Back to Player List" button were removed (the header `←` back arrow handles navigation; the team chip shows the team).
- **S2 partial-metrics notice** — when performance stats are absent but the player has renderable content (recorded skill ratings or clips), `#noStatsNotice` reads "Some of the performance metrics are not available yet."; with no content at all it keeps the client default "Performance metrics are not available yet."
...
- **S6:** Assessed card per-skill rows show the **full skill name** (Feature 045; abbreviations remain on S1/S8 only).
