...
- Playwright must not create extra positions under `sport_soccer`; use a disposable QA sport instead. Purge orphans with `node scripts/purge-soccer-position-orphans.js` (allowlist in `tests/playwright/_soccer-positions.js`).
