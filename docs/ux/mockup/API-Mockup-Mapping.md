...
- **S6 Open original + Re-process** — `GET /v1/clips` and share clip payloads include `sourceUrl` when the clip was link-ingested. S6 cards with a non-empty `sourceUrl` show **Open original** (`[data-testid="open-original-link"]`) opening that URL in a new tab (`target="_blank"` + `rel="noopener noreferrer"`). Failed clips show **Re-process** (`[data-testid="reprocess-clip"]`) for signed-in Coach / ClubAdmin / SystemAdmin only (guests keep a disabled Failed control). `POST /v1/clips/{id}/reprocess` with `{ actorEmail }` resets a failed clip to `submitted`, clears error/completion fields, and triggers the processing queue (202 + updated clip).
