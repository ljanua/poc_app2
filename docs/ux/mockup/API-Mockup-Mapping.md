...
- **S4 Link ingest (backlog 018)** — source mode Upload | From Link. Link mode posts multipart fields `videoUrl`, `startMmSs`, `durationMmSs` (default `01:00`, max `02:00`), and `findPlayer` instead of a video file. Backend downloads via yt-dlp (hosted) or HTTP (direct), then **first** extracts Start+Duration re-encoded at **≤ 30 fps**. When `findPlayer=true`, Ollama match runs **only inside that extract**; on match, assessment uses a further trim from the match offset (absolute `find_player_matched_ms` = start + offset). File uploads are also normalized to ≤ 30 fps before segmentation/AI. Columns: `source_url`, `source_start_ms`, `source_duration_ms`, `find_player`, `find_player_matched_ms`. Link mode requires backend (`DATABASE_URL`); offline mock keeps file upload only. UI: `[data-testid="source-mode-link"]`, `[data-testid="find-player-toggle"]` (default ON when avatar present).
