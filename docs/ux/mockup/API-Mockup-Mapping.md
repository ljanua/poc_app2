...
- **S4 Link ingest (backlog 018)** — source mode Upload | From Link. Link mode posts multipart fields `videoUrl`, `startMmSs`, `durationMmSs` (default `01:00`, max `02:00`), and `findPlayer` instead of a video file. Backend downloads via yt-dlp (hosted) or HTTP (direct), then trims. When `findPlayer=true`, processing seeks to Start, scans frames against `players.player_avatar_url` via Ollama, and extracts Duration only from the first match (fails if never found). Columns: `source_url`, `source_start_ms`, `source_duration_ms`, `find_player`, `find_player_matched_ms`. Link mode requires backend (`DATABASE_URL`); offline mock keeps file upload only. UI: `[data-testid="source-mode-link"]`, `[data-testid="find-player-toggle"]` (default ON when avatar present).
