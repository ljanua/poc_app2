...
| S10-games.html | List / create fixtures | Games CRUD | GET/POST /v1/games ; GET/PATCH /v1/games/{gameId} | 200/201 with `{ games }` / `{ game }`; Coach/ClubAdmin club-scoped; SystemAdmin unrestricted | 400 validation_error, 403 forbidden, 404 not_found |
| S10-games.html | Save Game Sheet | Complete sheet + rollup | PUT /v1/games/{gameId}/sheet | 200 with `{ game, substitutions, performances, minutesByPlayer }`; event-sourced minutes; upserts `game_performance`; recomputes `player_stats` match fields; does **not** write skill ratings / Assessment history | 400 validation_error, 403 forbidden, 404 not_found |
