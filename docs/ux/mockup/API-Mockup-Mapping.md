...
- S7 Users table includes a **Subscription** column (`tierDisplayName` / `tierCode`) for SystemAdmin and ClubAdmin viewers. **Change Subscription** is SystemAdmin-only (`POST /v1/admin/users/{userId}/subscription`); on success the user’s role is set via `roleForTierCode` unless the target is already SystemAdmin (tier updates, role preserved).
