...
- **S2 collapsible sections** — each remaining stats block title (`Development Progress`, `Match Time History`, `Recent Performance`, `Video Assessments`) is a `[data-testid="dashboard-section-toggle-*"]` button that toggles `is-collapsed` on the parent `.stats-section` (body hidden via `.section-body`). Default **collapsed** on first visit; expanded/collapsed state persists per player in `localStorage` key `vantageiq_s2_dashboard_sections` (`{ [playerId]: { [sectionSlug]: true|false } }`, `true` = expanded).
